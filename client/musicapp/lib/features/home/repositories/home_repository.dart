...
class HomeRepository {
  ...
  Future<Either<AppFailure, bool>> favSong(
      {required String token, required String song_id}) async {
    try {
      final res = await http.post(
          Uri.parse('${ServerConstants.serverUrl}/song/favorite'),
          headers: {'Content-Type': 'application/json', 'x-auth-token': token},
          body: jsonEncode({"song_id": song_id}));
  ...
  Future<Either<AppFailure, List<SongModel>>> getFavSongs(String token) async {
    try {
      final res = await http.get(
